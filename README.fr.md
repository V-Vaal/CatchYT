...
### 1. Depuis une release (rien à installer)

Va dans [Releases](https://github.com/V-Vaal/CatchYT/releases) et télécharge `catchyt.exe`. Le job `release` rattache l'artefact **qui a passé les tests** au lieu de recompiler : le binaire publié est celui qui a été testé.

### 2. Depuis un run de CI

Le workflow `.github/workflows/build.yml` compile et teste sur de vrais runners Windows, puis publie l'`.exe` en artefact. Onglet **Actions** du dépôt, dernier run `build` terminé, section **Artifacts**, télécharge `catchyt-windows-x86_64`. À noter : un artefact exige d'être connecté à GitHub et finit par expirer, contrairement à un asset de release.

### 3. Build local sur Windows
...
assets/                icônes
.github/workflows/     CI (build + test + artefact exe) et audit RUSTSEC
